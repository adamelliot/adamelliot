...
window.CanvasObject=window.CanvasObject||{};CanvasObject.Color=(function(){var Klass=function(r,g,b,a){this.red=Math.round(r||(isNaN(r)&&0));this.green=Math.round(g||(isNaN(g)&&0));this.blue=Math.round(b||(isNaN(b)&&255));this.alpha=a||(isNaN(a)&&1.0);this.r=function(val){this.red=val;return this;};this.g=function(val){this.green=val;return this;}
this.b=function(val){this.blue=val;return this;}
this.a=function(val){this.alpha=val;return this;}};Klass.prototype.toString=function(){return"rgba("+this.red+","+this.green+","+this.blue+","+this.alpha+")";};Klass.fromHex=function(hex,alpha){return new Klass(hex>>16,(hex>>8)&0xff,hex&0xff,alpha||1.0);};Klass.random=function(alpha){return Klass.fromHex(Math.floor(Math.random()*0xffffff),alpha);};Klass.randomGrey=function(alpha){var color=Math.floor(Math.random()*128)+120;return new Klass(color,color,color,alpha||1.0);};const COLOR_SETS={'black':[0,0,0],'clear':[0,0,0,0.0],'blue':[0,0,255],'red':[255,0,0],'green':[0,255,0],'yellow':[255,255,0],'orange':[255,128,0]};for(var color in COLOR_SETS)
...
if(children[i]==child)break;if(i==children.length)return;children.splice(i,1);this.removeHandler('enterFrame',child._enterFrame);};this.drawInto=function(context){for(var i=0;i<children.length;i++)
children[i].drawInto(context);}};Klass.prototype=new CanvasObject.Base;return Klass;})();CanvasObject.Stage=(function(){var Klass=function(target,_fps){var self=this;CanvasObject.Container.call(this);var interval,fps,currentFps=0,canvas,context;var noClearDraw,clearDraw,updateFps,drawFunc;var initialize=function(){if(!target)return;canvas=typeof target=='string'?document.getElementById(target):target;if(!canvas.getContext)throw"Uh oh! Can't get a context.";context=canvas.getContext('2d');if(!context)throw"Uh oh! Can't get a context.";drawFunc=clearDraw;this.enterFrame(drawFunc);this.setFrameRate(_fps);};noClearDraw=function(){self.drawInto(context);};clearDraw=function(){context.clearRect(0,0,canvas.width,canvas.height);self.drawInto(context);};var updateFps=function(){var lastTime=0,lastDiff=0;var i=0;updateFps=function(){var time=(new Date()).getTime();var diff=(time-lastTime);lastDiff=diff*0.5+lastDiff*0.5;lastTime=time;currentFps=Math.round(1000/lastDiff);};};updateFps();this.setUpdateMethod=function(clear,calcFps){this.removeHandler('enterFrame',drawFunc);this.removeHandler('enterFrame',updateFps);if(clear)this.enterFrame(drawFunc=clearDraw);else this.enterFrame(drawFunc=noClearDraw);if(calcFps)this.enterFrame(updateFps);};this.setFrameRate=function(value){if(!value||interval!=undefined)return;var ms=1000/(fps=value);interval=setInterval(function(){self.trigger('enterFrame');},ms);}
